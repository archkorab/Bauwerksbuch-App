...
async function generateInspectionPdf(inspection: any) {
  ...
  const shownDefectIds = new Set<number>();
  ...
  // Show orphan defects (not matched inline) and all defects if no bauteilprüfung
                        ...
                        {isInsExpanded && (
                          <div className="border-t border-border bg-muted/20 p-5 space-y-5" data-testid={`inspection-detail-${ins.id}`}>
                            ...
                            {(() => {
                              ...
                              const displayedBauteilNames = new Set(displayEntries.filter(e => !(headerNames.has(e.name) || !!e.isCustomHeader)).map(e => e.name));
                              const orphanDefects = primaryDefects.filter((d: any) => !d.bauteil?.some((b: string) => displayedBauteilNames.has(b)));
                              return (
                                <div className="bg-card rounded-xl border border-border p-4 space-y-3">
                                            ...
                                            <Fragment key={i}>
                                              ...
                                              {primaryDefects
                                                ...
                                                .map((defect: any) => {
                                                  const children = followUps.filter((f: any) => f.parentDefectId === defect.id);
                                                  return (
                                                    <Fragment key={`m-${defect.id}`}>
                                                      <tr className="border-b border-border bg-red-50/20 dark:bg-red-900/10">
                                                        <td colSpan={6} className="px-3 py-2.5">
                                                          <div className="ml-4 border-l-2 border-red-500/40 pl-4">
                                                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                              <span className="text-xs font-semibold text-red-600 dark:text-red-400 uppercase tracking-wider flex items-center gap-1">
                                                                <Hash className="w-3 h-3" />{defect.defectId}
                                                              </span>
                                                              <span className={`px-2 py-0.5 text-xs font-bold rounded-full border uppercase ${defect.status === 'grober_mangel' ? 'text-red-500 border-red-500/30 bg-red-500/10' : 'text-amber-500 border-amber-500/30 bg-amber-500/10'}`}>
                                                            ...
                                                            {(() => { const imgs: string[] = defect.imageUrls?.length ? defect.imageUrls : (defect.imageUrl ? [defect.imageUrl] : []); return imgs.length > 0 ? <div className="flex flex-wrap gap-1.5 mt-1.5">{imgs.map((src: string, ii: number) => <ExpandableImage key={ii} src={src} alt="Mangel" testId={`defect-image-${defect.defectId}-${ii}`} />)}</div> : null; })()}
                                                          </div>
                                                        </td>
                                                      </tr>
                                                      {children.map((child: any) => (
                                                        <tr key={`c-${child.id}`} className="border-b border-border bg-amber-50/10 dark:bg-amber-900/5">
                                                          <td colSpan={6} className="px-3 py-2">
                                                            <div className="ml-8 border-l-2 border-amber-500/30 pl-4">
                                                              <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                                ...
                                                                <span className={`px-2 py-0.5 text-xs font-bold rounded-full border uppercase ${child.status === 'grober_mangel' ? 'text-red-500 border-red-500/30 bg-red-500/10' : 'text-amber-500 border-amber-500/30 bg-amber-500/10'}`}>
                                                              ...
                                                              <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-muted-foreground">
                                                                {child.location && <span>Lage: {child.location}</span>}
                                                                {child.frist && <span>Frist: {fristLabels[child.frist] || child.frist}</span>}
                                                                {child.repairDue && <span>Reparatur bis: {format(new Date(child.repairDue), 'dd.MM.yyyy')}</span>}
                                                              </div>
                                                              {(() => { const imgs: string[] = child.imageUrls?.length ? child.imageUrls : (child.imageUrl ? [child.imageUrl] : []); return imgs.length > 0 ? <div className="flex flex-wrap gap-1.5 mt-1.5">{imgs.map((src: string, ii: number) => <ExpandableImage key={ii} src={src} alt="Mangel" testId={`defect-image-${child.defectId}-${ii}`} />)}</div> : null; })()}
                                                            </div>
                                                          </td>
                                                        </tr>
                                                      ))}
                                  ...
                                  {orphanDefects.length > 0 && (
                                    <div className="pt-2 space-y-2">
                                      ...
                                      {orphanDefects.map((defect: any) => {
                                        const children = followUps.filter((f: any) => f.parentDefectId === defect.id);
                                        return (
                                          <Fragment key={`o-${defect.id}`}>
                                            <div className={`rounded-lg border px-3 py-2.5 ${defect.status === 'grober_mangel' ? 'bg-red-50/20 border-red-500/20 dark:bg-red-900/10' : 'bg-amber-50/20 border-amber-500/20 dark:bg-amber-900/10'}`}>
                                              <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                ...
                                                <span className={`px-2 py-0.5 text-xs font-bold rounded-full border uppercase ${defect.status === 'grober_mangel' ? 'text-red-500 border-red-500/30 bg-red-500/10' : 'text-amber-500 border-amber-500/30 bg-amber-500/10'}`}>{defectStatusLabels[defect.status] || defect.status}</span>
                                            ...
                                            {children.map((child: any) => (
                                              <div key={`oc-${child.id}`} className="ml-6 rounded-lg border px-3 py-2 bg-amber-50/10 border-amber-500/20 dark:bg-amber-900/5">
                                                <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                  <CornerDownRight className="w-3 h-3 text-muted-foreground" />
                                                  <span className="text-xs font-semibold text-amber-600 uppercase tracking-wider flex items-center gap-1"><Hash className="w-3 h-3" />{child.defectId}</span>
                                                  <span className={`px-2 py-0.5 text-xs font-bold rounded-full border uppercase ${child.status === 'grober_mangel' ? 'text-red-500 border-red-500/30 bg-red-500/10' : 'text-amber-500 border-amber-500/30 bg-amber-500/10'}`}>{defectStatusLabels[child.status] || child.status}</span>
                                                </div>
                                                {child.description && <p className="text-sm text-foreground mb-1">{child.description}</p>}
                                              </div>
                                            ))}
                                    ...
                                    {primaryDefects.map((defect: any) => {
                                      ...
                                      return (
                                        <Fragment key={`fd-${defect.id}`}>
                                          ...
                                          {children.map((child: any) => (
                                            <div key={`fdc-${child.id}`} className="ml-6 rounded-lg border px-3 py-2 bg-amber-50/10 border-amber-500/20 dark:bg-amber-900/5">
                                              <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                <CornerDownRight className="w-3 h-3 text-muted-foreground" />
                                                <span className="text-xs font-semibold text-amber-600 uppercase tracking-wider flex items-center gap-1"><Hash className="w-3 h-3" />{child.defectId}</span>
                                                <span className={`px-2 py-0.5 text-xs font-bold rounded-full border uppercase ${child.status === 'grober_mangel' ? 'text-red-500 border-red-500/30 bg-red-500/10' : 'text-amber-500 border-amber-500/30 bg-amber-500/10'}`}>{defectStatusLabels[child.status] || child.status}</span>
