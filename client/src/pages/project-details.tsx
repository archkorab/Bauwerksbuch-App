...
async function generateInspectionPdf(inspection: any) {
  ...
  if (notes.includes("| Bauteilprüfung: ")) {
    ...
    if (entries.length > 0) {
      ...
      const IMG_W = 80;
      const IMG_H = 60;
      const defectsWithImgs = defectsInOrder.filter((d: any) => d.imageUrls?.length || d.imageUrl);
      if (defectsWithImgs.length > 0) {
        const defectImages: Map<number, string[]> = new Map();
        for (let di = 0; di < defectsWithImgs.length; di++) {
          const d = defectsWithImgs[di];
          ...
          if (loaded.length) defectImages.set(di, loaded);
        }
        for (let di = 0; di < defectsWithImgs.length; di++) {
          const d = defectsWithImgs[di];
          const imgList = defectImages.get(di) || [];
          if (!imgList.length) continue;
          const rowCount = Math.ceil(imgList.length / 3);
          const needed = rowCount * (IMG_H + 4) + 10;
          if (y + needed > pageHeight - 20) { doc.addPage(); drawHeader(); y = 33; }
          doc.setFont("helvetica", "italic");
          doc.setFontSize(7.5);
          doc.setTextColor(...PDF_COLORS.mutedFg);
          doc.text(`Fotos: ${d.defectId}`, margin + 2, y + 3);
          y += 6;
          for (let ii = 0; ii < imgList.length; ii++) {
            const xPos = margin + 2 + (ii % 3) * (IMG_W + 4);
            const yPos = y + Math.floor(ii / 3) * (IMG_H + 4);
            doc.addImage(imgList[ii], "JPEG", xPos, yPos, IMG_W, IMG_H);
          }
          y += rowCount * (IMG_H + 4) + 4;
        }
      }
      y += 4;
    }
                                                      ...
                                                      <tr className={`border-b border-border/40 ${isGrober ? 'bg-red-50/20 dark:bg-red-950/15' : 'bg-amber-50/10 dark:bg-amber-950/5'}`}>
                                                        <td colSpan={6} className="px-3 py-2.5">
                                                          <div className={`ml-4 pl-4 border-l-2 ${isGrober ? 'border-red-500/40' : 'border-amber-500/40'}`}>
                                                            ...
                                                            {imgs.length > 0 && <div className="flex flex-wrap gap-1 mt-1.5">{imgs.map((src: string, ii: number) => <ExpandableImage key={ii} src={src} alt="Mangel" testId={`defect-image-${defect.defectId}-${ii}`} />)}</div>}
                                                          </div>
                                                        </td>
                                                      </tr>
                                                          ...
                                                          <tr key={`followup-${child.id}`} className="bg-muted/10 border-b border-border/30">
                                                            <td colSpan={6} className="px-3 py-2">
                                                              <div className="ml-8 pl-4 border-l-2 border-muted-foreground/20">
                                                                ...
                                                                {childImgs.length > 0 && <div className="flex flex-wrap gap-1 mt-1.5">{childImgs.map((src: string, ii: number) => <ExpandableImage key={ii} src={src} alt="Mangel" testId={`defect-image-${child.defectId}-${ii}`} />)}</div>}
                                                              </div>
                                                            </td>
                                                          </tr>
                                                        );
