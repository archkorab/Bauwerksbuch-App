...
import { useState } from 'react'
import { signUp, logIn, forgotPassword } from '../lib/auth'
  ...
  return (
    <div style={{ maxWidth: 400, margin: '100px auto', padding: 24 }}>
      ...
      <div style={{ marginTop: 16, fontSize: 14 }}>
        {mode === 'login' && <>
          <span onClick={() => setMode('signup')} style={{ cursor: 'pointer', color: 'blue' }}>Create an account</span>
          {' · '}
          <span onClick={() => setMode('forgot')} style={{ cursor: 'pointer', color: 'blue' }}>Forgot password?</span>
        </>}
        {mode === 'signup' && (
          <span onClick={() => setMode('login')} style={{ cursor: 'pointer', color: 'blue' }}>Already have an account? Log in</span>
        )}
        {mode === 'forgot' && (
          <span onClick={() => setMode('login')} style={{ cursor: 'pointer', color: 'blue' }}>Back to login</span>
        )}
      </div>
    </div>
  )
